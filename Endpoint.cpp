...
Endpoint::Endpoint(const sockaddr_storage &storage_, const socklen_t len) {
    if (len >= static_cast<int>(sizeof(sockaddr_in6)) && storage_.ss_family == AF_INET6) {
        sockaddr_in6 addr6{};
        std::memcpy(&addr6, &storage_, sizeof(sockaddr_in6));
        this->storage = IPv6Addr{addr6};
    } else if (len >= static_cast<int>(sizeof(sockaddr_in)) && storage_.ss_family == AF_INET) {
        sockaddr_in addr{};
